...
public class SNAPParser {
    ...
    static HashMap<Integer,SNAPNode> nodeList = new HashMap<Integer, SNAPNode>(); //should change to list
...
//    }
    ...
    private static void directedRankPages(){
        ...
        for(Map.Entry<Integer, SNAPNode> entry : nodeList.entrySet()) {
            next = entry.getValue();
            next.pageRank = 1.0/nodeList.size();
            next.oldrank = 1.0/nodeList.size();
            nodeList.put(next.identifier, next);
        }
        ...
        for(r = 0; r < 1000; r++){
            for(Map.Entry<Integer, SNAPNode> entry : nodeList.entrySet()){
                ...
                for(Integer edge : next.inEdges){
                    ...
                    linkProb += (1.0/other.edges) * other.oldrank;
                }
                next.oldrank = next.pageRank;
